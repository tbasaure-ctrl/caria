...
class Settings:
    ...
    def resolve_path(self, relative_path: str | Path) -> Path:
        """Resuelve un path relativo a un path absoluto.
        
        Si el path es relativo, intenta resolverlo relativo a caria_data/
        buscando en múltiples ubicaciones comunes.
        
        Args:
            relative_path: Path relativo o absoluto
        ...
        # Si ya es absoluto, retornarlo
        if path.is_absolute():
            return path
        
        # Buscar en ubicaciones comunes relativas a caria_data
        # Intentar encontrar caria_data desde el módulo actual
        current_file = Path(__file__).resolve()
        # Desde caria/config/settings.py, subir hasta caria_data/
        caria_data_root = current_file.parent.parent.parent.parent
        
        # Posibles ubicaciones base
        possible_bases = [
            caria_data_root,  # caria_data/
            caria_data_root.parent / "caria_data",  # notebooks/caria_data/
            Path.cwd(),  # Directorio actual de trabajo
            Path.cwd() / "caria_data",  # caria_data/ desde cwd
        ]
        
        # Buscar el path en cada base posible
        for base in possible_bases:
            full_path = base / path
            if full_path.exists():
                return full_path.resolve()
        
        # Si no se encuentra, retornar relativo al primer base (caria_data/)
        return (possible_bases[0] / path).resolve()
