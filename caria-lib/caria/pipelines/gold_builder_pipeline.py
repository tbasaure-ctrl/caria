...
def _compute_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # NOTE: target_regime removed - regime detection will be done via HMM (Sistema I) in future
    return df
...
def gold_builder_flow(settings: Settings, config_path: str) -> None:
    ...
    for dataset in datasets:
        ...
        # Skip events datasets - regime detection will be done via HMM (Sistema I) in future
        if dataset.get("type") == "events":
            LOGGER.info("Skipping events dataset %s - regime detection will use HMM", rel_path)
            continue
