...
DEPRECATED: Este pipeline genera etiquetas ex-post facto que introducen look-ahead bias.
El régimen macro será detectado usando HMM no supervisado (Sistema I) en el futuro.
Este módulo se mantiene solo para referencia histórica y será eliminado completamente.
"""
...
def aggregate_news_sentiment(raw_root: Path) -> pd.DataFrame:
    ...
    if "ticker" in combined.columns:
        aggregated = (
            combined.groupby([pd.to_datetime(combined["date"]), combined["ticker"]])
            ["sentiment_score"]
            .mean()
            .reset_index()
        )
        ...
        aggregated = (
            combined.groupby(pd.to_datetime(combined["date"]))["sentiment_score"].mean().reset_index()
        )
        aggregated["ticker"] = None

    aggregated.rename(columns={"sentiment_score": "sentiment_score"}, inplace=True)
    aggregated["source"] = "newsapi"
    aggregated["event_tags"] = aggregated.get("ticker", pd.Series([None] * len(aggregated))).apply(
        lambda _: json.dumps([])
    )
