...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'community_posts' AND column_name = 'arena_thread_id'
    ) THEN
        ALTER TABLE community_posts ADD COLUMN arena_thread_id UUID REFERENCES thesis_arena_threads(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'community_posts' AND column_name = 'arena_round_id'
    ) THEN
        ALTER TABLE community_posts ADD COLUMN arena_round_id UUID REFERENCES arena_rounds(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'community_posts' AND column_name = 'arena_community'
    ) THEN
        ALTER TABLE community_posts ADD COLUMN arena_community VARCHAR(50);
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'community_posts' AND column_name = 'is_arena_post'
    ) THEN
        ALTER TABLE community_posts ADD COLUMN is_arena_post BOOLEAN DEFAULT FALSE;
    END IF;
END $$;
