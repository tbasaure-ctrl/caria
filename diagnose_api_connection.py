...
def main():
    ...
    if results["health"]["status"] == "success":
        ...
        print(f"   ❌ Backend no responde: {results['health'].get('error', results['health'])}")
    print()
    ...
    if results["cors"]["status"] == "success":
        ...
        print(f"   ⚠️  CORS test falló: {results['cors'].get('error', results['cors'])}")
    print()
    
    # 3. FMP API
    print("3️⃣  Verificando FMP API (precios)...")
    results["fmp"] = test_fmp_connection()
    if results["fmp"]["status"] == "success":
        print("   ✅ FMP API funciona correctamente")
        if "data" in results["fmp"] and isinstance(results["fmp"]["data"], dict):
            prices = results["fmp"]["data"].get("prices", {})
            if prices:
                print(f"   📊 Precio de prueba obtenido: {list(prices.keys())[0]}")
    else:
        print(f"   ❌ FMP API no funciona: {results['fmp'].get('error', results['fmp'])}")
        if results["fmp"].get("status_code") == 500:
            print("   ⚠️  Error 500: FMP_API_KEY probablemente no está configurada en Cloud Run")
    print()
    
    # 4. Gemini API
    print("4️⃣  Verificando Gemini API (análisis)...")
    results["gemini"] = test_gemini_connection()
    if results["gemini"]["status"] == "success":
        print("   ✅ Gemini API funciona correctamente")
        ...
        if results["gemini"]["status_code"] in [401, 403]:
            ...
            print("   ❌ Error 500: GEMINI_API_KEY probablemente no está configurada en Cloud Run")
        else:
            print(f"   ⚠️  Status {results['gemini']['status_code']}: {results['gemini'].get('data', '')}")
    else:
        print(f"   ❌ Gemini API no funciona: {results['gemini'].get('error', results['gemini'])}")
    print()
    
    # 5. Reddit API
    print("5️⃣  Verificando Reddit API (sentimiento)...")
    results["reddit"] = test_reddit_connection()
    if results["reddit"]["status"] == "success":
        print("   ✅ Reddit API funciona correctamente")
    else:
        print(f"   ❌ Reddit API no funciona: {results['reddit'].get('error', results['reddit'])}")
        if results["reddit"].get("status_code") == 500:
            print("   ⚠️  Error 500: REDDIT_CLIENT_ID/SECRET probablemente no están configuradas en Cloud Run")
    print()
    
    # 6. Fear & Greed Index
    print("6️⃣  Verificando Fear & Greed Index...")
    results["fear_greed"] = test_fear_greed_index()
    if results["fear_greed"]["status"] == "success":
        print("   ✅ Fear & Greed Index funciona correctamente")
    else:
        print(f"   ❌ Fear & Greed Index no funciona: {results['fear_greed'].get('error', results['fear_greed'])}")
    print()
    ...
    print(f"\n✅ Funcionando: {working}/{total}")
    print(f"❌ Con problemas: {total - working}/{total}\n")
    
    # Recomendaciones
    if results["health"]["status"] != "success":
        print("🔴 PROBLEMA CRÍTICO: El backend no está corriendo")
    ...
    if results["reddit"].get("status_code") == 500:
        print("\n🔴 REDDIT_CLIENT_ID/SECRET no configuradas:")
