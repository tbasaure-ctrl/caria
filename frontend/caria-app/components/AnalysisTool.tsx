...
export const AnalysisTool: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [riskRewardTicker, setRiskRewardTicker] = useState<string>('');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Update risk-reward panel ticker if found
        if (ticker) {
            setRiskRewardTicker(ticker);
        }
        ...
        >
            <div
                className={`relative flex flex-col h-full max-h-[90vh] w-full ${
                    showRiskReward ? 'max-w-7xl' : 'max-w-4xl'
                ...
                <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                    ...
                    {showRiskReward && (
                        <div className="w-full lg:w-1/2 flex flex-col">
                            <RiskRewardPanel 
                                ticker={riskRewardTicker}
                                onTickerChange={setRiskRewardTicker}
                            />
                        </div>
                    )}
                </div>
