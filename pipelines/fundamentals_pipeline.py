...
def _manual_metrics_from_statements(
    ...
    income = income[[c for c in cols_income if c in income.columns]].rename(columns=cols_income)
    balance = balance[[c for c in cols_balance if c in balance.columns]].rename(columns=cols_balance)
    cash = cash[[c for c in cols_cash if c in cash.columns]].rename(columns=cols_cash)
    ...
    merged["net_income_growth"] = merged["net_income"].pct_change()

    ebit = merged["ebit"].fillna(merged["operating_income"]).fillna(merged["net_income"])
    tax_rate = np.where(
        merged["income_before_tax"].abs() > 1e-6,
        (merged["income_tax"].abs() / merged["income_before_tax"].abs()).clip(0, 1),
        0.21,
    )
    merged["nopat"] = ebit * (1 - tax_rate)

    merged["invested_capital"] = (
        merged.get("total_debt", 0).fillna(0)
        + merged.get("total_equity", 0).fillna(0)
        - merged.get("cash_equivalents", 0).fillna(0)
    )
    ...
    merged["gross_profit_margin"] = np.where(
        ...
        merged.get("gross_profit").fillna(0) / merged["revenue"],
        np.nan,
    )
    ...
    merged["return_on_assets"] = np.where(
        merged.get("total_assets", 0).abs() > 1e-6,
        merged["net_income"] / merged.get("total_assets"),
        np.nan,
    )
    merged["return_on_equity"] = np.where(
        merged.get("total_equity", 0).abs() > 1e-6,
        merged["net_income"] / merged.get("total_equity"),
        np.nan,
    )

    merged["net_debt"] = merged.get("total_debt", 0).fillna(0) - merged.get("cash_equivalents", 0).fillna(0)
    merged["capital_expenditure"] = merged.get("capital_expenditure", np.nan)
    merged["research_and_development"] = merged.get("research_and_development", np.nan)
    merged["free_cash_flow"] = merged.get("free_cash_flow").fillna(
        merged.get("operating_cash_flow", np.nan) + merged.get("capital_expenditure", np.nan)
    )

    shares = merged.get("shares_diluted").fillna(merged.get("shares_outstanding")).fillna(
        merged.get("shares_basic")
    )
